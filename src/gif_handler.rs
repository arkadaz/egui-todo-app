    ...
    pub fn tick(&mut self, ctx: &eframe::egui::Context) {
        if let Some(gif) = &mut self.gif {
                ...
                .is_none()
            {
                ...
                gif.textures[self.current_frame] =
                    Some(ctx.load_texture(name, frame_data, Default::default()));
            }
                ...
                .and_then(|t| t.as_ref())
            {
                egui::CentralPanel::default()
                    ...
                    .show(ctx, |ui| {
                        ...
                        let ratio =
                            (available.width() / img_size.x).min(available.height() / img_size.y);
