    ...
    egui::Window::new(window_title)
        ...
        .show(ctx, |ui| {
            ...
            egui::Grid::new("calendar_grid").spacing([4.0, 4.0]).show(ui, |ui| {
                ...
                for _ in 0..weekday_offset { ui.label(""); }

                let days_in_month = NaiveDate::from_ymd_opt(calendar_date.year(), calendar_date.month() + 1, 1)
                    .unwrap_or_else(|| NaiveDate::from_ymd_opt(calendar_date.year() + 1, 1, 1).unwrap())
                ...
                for day in 1..=days_in_month {
                    ...
                    let has_todos = todos_by_date.get(&current_cell_date).is_some_and(|v| !v.is_empty());

                    let mut frame = egui::Frame::central_panel(ui.style()).inner_margin(egui::Margin::same(4.0));
                    if is_today { frame = frame.fill(ui.visuals().selection.bg_fill); }
                    if is_selected { frame = frame.stroke(ui.visuals().widgets.active.bg_stroke); }

                    frame.show(ui, |ui| {
                        ui.centered_and_justified(|ui| {
                            let mut label = egui::RichText::new(day.to_string());
                            if is_today { label = label.color(ui.visuals().selection.stroke.color).strong(); }
                            let response = ui.add(egui::Label::new(label).sense(egui::Sense::click()));
                            if response.clicked() { *selected_date = current_cell_date; }
                            if has_todos {
                                let dot_pos = response.rect.center_bottom() + egui::vec2(0.0, -2.0);
                                ui.painter().circle_filled(dot_pos, 2.0, ui.visuals().text_color());
                            }
                        });
                    });
                    if (day + weekday_offset) % 7 == 0 { ui.end_row(); }
                }
            });
        });
    *is_open = open;
